...
- `DESIGN.md` の Apple ライクなトークンを **PressNote 用に再解釈**:
  - アクセント色は `{colors.primary}` （#0066cc）を踏襲
  - **日本語フォントは Noto Sans JP 固定**（プロジェクト指示）。英数は `system-ui, -apple-system`
  - フルブリードのタイル交互配色、影は記事サムネイル（あれば）のみ
  - ボタンは pill（プライマリ） + rect（管理画面ユーティリティ）の 2 種類
- 通知バナーは Parchment 背景 + ピル CTA で控えめに
