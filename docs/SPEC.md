...
- 管理者は `/admin/sources/new` で「対象企業名」と「スクレイピング対象 URL」を入力し、「登録作業に入る」から設定画面へ進む。
- 設定画面では対象 URL の HTML をアプリ内プレビューとして表示し、管理者がクリックした要素から CSS セレクタを設定できる。
- 設定対象は `list.item`(記事要素)、`list.date`(日付要素)、`list.title`(タイトル要素)、`list.url`(記事への URL)、`detail.body`(本文要素)、`detail.followLinks[]`(追加遷移/PDF リンク) とする。
- クロール時は保存済みセレクタに沿って一覧から記事候補を抽出する。RSS/Atom が見つかる場合は RSS を優先し、RSS がない企業ページは `scrape` として保存する。
- AI 解析時は記事 URL の先が HTML でも PDF でも本文を取得する。HTML 詳細ページ内に PDF などの追加リンクがある場合は `detail.followLinks[]` の設定に従って追加取得し、要約入力へ含める。
- 初回バックフィルのデフォルトは 5 件とする。
