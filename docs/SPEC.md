...
> 監視対象企業のプレスリリースを毎日収集し、AI による要約・用語解説とともに配信する Web アプリ。
...
3. 興味のあるリリースをクリックし、AI 要約 + 用語解説で内容を理解する。
...
│ ナビ   ├────────────────────────────────────────┤              │
│ レール │ フィルタチップ + 並び替え               │ 注目企業      │
│        ├────────────────────────────────────────┤ 今日のトピック │
│        │ プレスリリースカード(縦リスト)          │ 用語解説      │
│        │  - 企業ロゴ + 企業名 + 日付             │ トレンドタグ  │
│        │  - タイトル                            │              │
│        │  - AI 要約 (短)                        │              │
│        │  - カテゴリ/タグチップ                  │              │
...
- 左ナビ: アイコン + ラベル。「ホーム / モニター / アラート / トピック / ヘルプ」想定 (初期実装は「ホーム」「企業一覧」「管理」のみ、他はプレースホルダで可)。
- ヘッダ: ページタイトル「今日のプレスリリース」 + 日付ピッカー + 検索 + ホーム/企業一覧/ヘルプ アイコン。
- 中央リスト:
...
- 右ウィジェット(PC のみ表示、tablet 以下では下部へスタック):
  - **注目企業**: 直近 7 日でリリース数が多い企業 Top 5。
  - **今日のトピック**: 当日のタグ出現頻度 Top 5。
  - **用語解説**: 当日記事から抽出した用語ランダム 5 件。
  - **トレンドタグ**: 過去 30 日のタグ Top 10。
...
- フロー:
  ...
  3. ユーザーは **時刻 (HH:MM, 30 分単位 = `:00` / `:30` のみ)** を設定可。タイムゾーンはブラウザから自動取得(IANA 名で保存)。デフォルト 09:00。UI は 30 分刻みのセレクタで提示する。
...
- ユーザー設定 UI: トップ画面右上の通知アイコンから時刻設定モーダル(30 分刻みのプルダウン)。
...
- Structured Outputs (`response_format: { type: "json_schema" }`) で固定スキーマ:
  ...
  ```json
  {
    ...
    "terms": [{ "term": "用語", "description": "1〜2文の解説" }],
    "tags": ["業界タグ", "..."]
  }
  ```
...
- **出力言語**: 入力が英語など日本語以外であっても、`summary_short` / `summary_long` / `terms[].description` は **常に日本語** で生成する(プロンプトで強制)。元の `title` は原文のまま保持する(UI で原文タイトルを表示)。
...
| terms                              | jsonb nullable       | **永続** `[{term, description}]`                               |
...
- anon ロールは `articles(hidden_at IS NULL)` と `sources(enabled=true, deleted_at IS NULL)` の `SELECT` のみ許可。
...
| 対象                                                   | 保持期間                        |
| ------------------------------------------------------ | ------------------------------- |
| 記事 URL / AI 要約 / 用語 / タグ / タイトル / 公開日時 | **無期限(永続)**                |
...
| M3: AI 解析        | OpenAI `gpt-5.5` 連携、要約・用語解説・タグ保存、再試行                                              |
...
以下の制約を守って JSON で要約・用語解説・タグを返してください。

【出力ルール】
- 出力は必ず日本語(常体ではなく「です・ます」も使わず、中立的な常体「〜である」「〜した」)。
- 入力が英語などの場合も、要約・用語解説は日本語に翻訳して書く。
