...
- TOPページは、直近24時間の新着プレスリリースが0件の場合でも否定的な空状態にしない。
- ステータスヘッダーには「新着のプレスリリースはありません」「直近24時間の新着はありません。次回のチェックは HH:00 です。」を表示する。
- 次回チェック時刻は、クライアント側で JST 06:00 / 12:00 / 18:00 の探索スケジュールから計算する。追加のDB項目は持たない。
- ステータスヘッダーには、現在有効な監視ソース数として「現在 N 社を監視中」を表示する。
- 「今日の用語」は `terms` から日付ハッシュで決定的に1件を選ぶ。同じJST日付では同じ用語を表示し、翌日0:00以降に自然に切り替わる。
- 「今日の用語」には、見出し語、読み、意味、関連する記事数へのリンクを表示する。関連件数は `article_terms` の件数を使用する。
- 「最近のプレスリリース」は、直近24時間の新着がある場合はその新着を表示する。直近24時間が0件の場合は、直近6件の記事にフォールバックして「最近のプレスリリース（新着はまだありません）」として表示する。
- 最近のプレスリリースは既存の記事カードを再利用し、JSTの日付ごとに区切って表示する。
- 記事履歴も0件の場合は、探索完了後に表示されることが分かる空状態を表示する。
