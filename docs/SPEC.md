...
- サイト紹介を主目的にせず、サイトマップ（最新 / 記事一覧 / 用語帳 / 通知設定）を掲載する
- 運用リンクとして GitHub リポジトリ、README、仕様書への外部リンクを掲載する
- 要約は AI 生成であり、正確な内容はリンク先の原文を確認する旨を注記する
- 不具合報告は右下アイコンから匿名で送信できる旨を補足する
- 管理画面へのリンクはフッターに掲載しない
...
- **LLM コストグラフ**: OpenAI Costs API から当月 1 日〜末日の日次合計コストを取得し、`USD_TO_JPY_RATE` で円換算して横軸を日付、縦軸をコスト（円）とする棒グラフで表示。月次合計も円でサマリー表示し、ツール別・input/output 別の内訳は表示しない
...
- 管理画面の用語一覧は見出し語・読み方で検索できる。頭文字フィルタリングは管理画面には表示しない。
- 新規追加・編集（見出し語・読み方・解説）
...
#### 2.3.1 クロール（1 日 1 回: 06:00 JST）

- 各有効ソースに対して OpenAI Responses API + `web_search` ツールで、登録 URL の本文主コンテンツ領域から **JST 当日・前日公開のプレスリリースのみ** を取得する
- Cron 実行時は件数の上限・下限を設けない。取得できた対象日分だけを処理し、指定件数を満たすための追加検索・再試行は行わない
- ヘッダー、グローバルナビ、フッター、サイドバー、関連記事、別カテゴリのニュースリンクにある URL は除外する
- 取得した URL のうち、`articles.url` に存在しないもののみ AI 要約処理へ進め、要約結果の `published_at` が JST 当日または前日であることを保存前に再検証する
- 記事URLは保存前・表示前に正規化する。ハッシュ、`utm_*`、`gclid` などの計測パラメータ、ホスト名の大小、末尾スラッシュ差分は同じ遷移先として扱い、同じ記事が重複表示・重複保存されないようにする。
...
- 用語抽出は業界固有の専門語、技術方式、制度名、規格名、専門的な略語に限定する。一般的なビジネス語、単独の一般英単語・カタカナ語、説明なしでも理解しやすい語は抽出しない。迷う場合は抽出しない。
...
| -------- | ------------------------------- | ---------------------------------------------- | -------------------------------- |
| GET      | `/api/cron/crawl?source=<uuid>` | **指定 1 ソース** のクロール + 要約 + 用語抽出 | `0 21 * * *` (UTC) = JST 06:00 に Supabase 側で有効ソースを fanout |
...
探索は HTML パースではなく、OpenAI Responses API + `web_search` ツールに 1 プロンプトで指示する。

Cron 実行時:

1. 登録 URL を調べ、ページ本文の主コンテンツ領域にある記事一覧・リスト・カードから JST 当日・前日公開のプレスリリース個別 URL のみを取得
2. 対象日公開と確認できない URL、公開日が不明な URL、対象日より前の URL は返さない
3. 対象日分が見つからない場合は追加検索や再試行で無理に探しに行かず、空配列を返す
...
- プロンプト: 「次の URL の本文を取得し、90〜110 文字程度の要約と専門用語（読み方つき）を抽出」。用語は読者が調べる価値のある専門語に限定し、一般語や単独の英語表現は除外する。
- 指定 URL の本文を確認できない場合や、検索結果・関連掲載からの推定しかできない場合は `is_press_release=false` とし、推定で要約を補わない。
- `title` と `summary` には「要約」「抽出」「参考」「取得できず」「推定」「文字数」など、AI の作業過程・取得失敗・注意書き・文字数説明に関するメタ情報を含めない。
...
- 右下の固定ボタンは「フィードバック」とし、不具合報告と機能要望を選択して送信できる。
- 右下の固定ボタン内のフィードバックアイコンは、ボタン領域に対して視覚的に中央に見えるよう、アイコン図形の path 座標を上方向に 2px 相当調整する。
- 不具合報告は本文、パス、ブラウザ情報、表示サイズ、言語、タイムゾーン、直近のエラーログを送信する。
- 機能要望は本文とパスのみを必須情報として送信し、コンソールログは送信しない。
- 管理画面 `/admin/bug-reports` は不具合報告と機能要望を同じページに表示し、種別バッジで一目で判別できるようにする。
- 管理画面では「すべて / 不具合報告 / 機能要望」でフィルターできる。
- DB は `bug_reports.kind` に `bug` または `feature` を保存する。既存行は `bug` とみなす。

## 20. TOPページの状態表示強化

- TOPページは、直近1週間のプレスリリースを日付ごとに表示する。
- TOPページの表示順は、日替わり用語ピックアップ、直近1週間の記事、通知導線の順とする。監視中ソース数や「確認しました」状態カードはTOPには表示しない。
- 直近1週間の記事セクションの見出しは「最近のプレスリリース」とする。
- ステータスヘッダーには「この1週間のプレスリリースはありません」「直近1週間の記事はありません。次回のチェックは HH:00 です。」を表示する。
- 次回チェック時刻は、クライアント側で JST 06:00 / 12:00 / 18:00 の探索スケジュールから計算する。追加のDB項目は持たない。
- ステータスヘッダーには、現在有効な監視ソース数として「現在 N 社を監視中」を表示する。
- 「日替わり用語ピックアップ」は `terms` から日付ハッシュで決定的に1件を選ぶ。同じJST日付では同じ用語を表示し、翌日0:00以降に自然に切り替わる。
- 「日替わり用語ピックアップ」には、見出し語、読み、意味、関連する記事数へのリンクを表示する。関連件数は `article_terms` の件数を使用する。
