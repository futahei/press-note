...
- 各有効ソースに対して OpenAI Responses API + `web_search` ツールで、登録 URL の本文主コンテンツ領域から **JST 当日公開のプレスリリースのみ** を取得する
- Cron 実行時は件数の上限・下限を設けない。取得できた当日分だけを処理し、指定件数を満たすための追加検索・再試行は行わない
- ヘッダー、グローバルナビ、フッター、サイドバー、関連記事、別カテゴリのニュースリンクにある URL は除外する
- 取得した URL のうち、`articles.url` に存在しないもののみ AI 要約処理へ進め、要約結果の `published_at` が JST 当日であることを保存前に再検証する
...
1. 登録 URL を調べ、ページ本文の主コンテンツ領域にある記事一覧・リスト・カードから JST 当日公開のプレスリリース個別 URL のみを取得
2. 当日公開と確認できない URL、公開日が不明な URL、前日以前の URL は返さない
3. 当日分が見つからない場合は追加検索や再試行で無理に探しに行かず、空配列を返す
