...
const summaryJsonSchema = {
  ...
  properties: {
    ...
    summary: {
      ...
      maxLength: 120,
      description:
        "記事本文の要約。日本語で90〜110文字程度。取得失敗、推定、要約作業、文字数に関するメタ説明を含めない。"
    },
    published_at: {
    ...
    input: [
      {
        role: "system",
        content:
          "あなたは日本語のプレスリリース編集者です。指定されたURLそのものの本文を確認し、プレスリリースだけを日本語90〜110文字程度で要約し、読者がつまずく専門用語を抽出してください。指定URLの本文を確認できない、別ページや検索結果からの推定しかできない、またはプレスリリース本文だと確証できない場合は is_press_release=false、terms=[] にしてください。title は記事本文の正式タイトル、または本文内容だけから作る自然な記事見出しにしてください。「ニュース本文要約」「専門用語抽出」など処理名を title に入れないでください。summary は短すぎる箇条書きではなく、本文の要点が分かる1〜2文にしてください。title と summary には、取得失敗、URLにアクセスできない、関連掲載から推定、参考、要約したこと、文字数、専門用語抽出、AIの作業過程、制約、注意書き、出典確認の説明などのメタ情報を絶対に含めないでください。published_at は ISO 8601 datetime with timezone で返し、公開日時が取得できない場合は null にしてください。terms は読者が調べる価値のある業界固有の専門語、技術方式、制度名、規格名、専門的な略語に限定してください。単独の一般英単語、一般的なカタカナ語、一般的なビジネス語、会社名・人名・地名だけの語、記事固有のキャンペーン名、説明なしでも理解しやすい語は抽出しないでください。例: AI、DX、クラウド、サービス、システム、アプリ、データ、プラットフォームだけを単独では抽出しないでください。迷う場合は抽出しないでください。description は記事文脈を持ち込まず、その用語だけで独立して理解できる定義にしてください。"
      },
      {
        role: "user",
        content: `次のURLそのものの本文を取得し、90〜110文字程度の要約と専門用語を抽出してください。指定URLの本文を確認できない場合は推定で補わず、is_press_release=false にしてください。title と summary には「要約」「抽出」「参考」「取得できず」「推定」「文字」など、処理や失敗に関する説明を書かないでください。一般語や単独の英語表現は terms に入れないでください: ${url}`
      }
    ],
    text: {
      format: {
